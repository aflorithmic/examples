...
def multiple_replace(
    dict, text
):  # function to replace using a dictionary rather then just single strings
...
def main(args):
    ...
    song = genius_api.search_song(
        title=args.song, artist=args.artist, song_id=None, get_full_info=True
    )  # find the wanted song from a given artist
    ...
    script = apiaudio.Script().create(
        ...
        + lyrics[
            :500
        ],  # full songs with background track can be quite difficult to handle thus we only render the first 500 characters
    )
    ...
    audio_files = apiaudio.Speech().download(
        scriptId=script["scriptId"], destination="."
    )  # download raw speech
    ...
    )  # download mastered speech


if __name__ == "__main__":
    parser = ArgumentParser()
    parser.add_argument(
        "-a", "--artist", type=str, required=True, help="artist of the song",
    )
    parser.add_argument(
        "-s", "--song", type=str, required=True, help="song name",
    )
    parser.add_argument(
        "-v",
        "--voice",
